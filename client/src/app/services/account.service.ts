...
export class AccountService {
  ...
  async login(email: string, password: string): Promise<void> {
    ...
    if (hitUser === null)
      throw new Error('指定した UID に対応するユーザーが見つかりません');

    localStorage.setItem('myself', JSON.stringify(hitUser));
  }

  get uid() {
    return this.loginSubject.value && this.loginSubject.value.uid;
  }

  get name() {
    return this.loginSubject.value && this.loginSubject.value.name;
  }

  get screenName() {
    return this.loginSubject.value && this.loginSubject.value.screenName;
  }

  get image() {
    return this.loginSubject.value && this.loginSubject.value.image;
  }

  get bio() {
    return this.loginSubject.value && this.loginSubject.value.bio;
  }
}
